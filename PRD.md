...
| **v1** | Custom `FinderComponent` (§7): snippets, live filter, scope toggle, `onProgress` status. Tests. **Shipped: rich preview pane ✅ (item 1), recap-at-landing ✅ (item 7). Pending: peek `>`/`<` paging (item 5).** Settings are env-var knobs (`PI_FIND_*`) until pi exposes a config API. See `PLAN.md`. | Production polish. |
